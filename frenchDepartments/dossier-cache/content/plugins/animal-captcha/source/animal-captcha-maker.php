...
if (($ac_maker_active == true) OR (isset($ac_dir))) {
	...
	if (isset($ac_dir)) { // requested from wp backend
		$in_wp = true;
		$ac_dir_img = plugins_url('animal/',__FILE__);
	} else { // direct request
		$in_wp = false;
		$ac_dir = 'animal/';
		$ac_dir_img = $ac_dir; 
	...
	echo '<h1><a href="'.($in_wp? 'plugins.php?page=animal-captcha-maker':'animal-captcha-maker.php').'">Animal Captcha Maker</a></h1>
	...
	switch ($_REQUEST['a']) { // a = make action
		...
		case 'add_image':
			...
			echo '<blockquote>
	<h2>Add new image</h2>
	<form action="'.($in_wp?'plugins.php?page=animal-captcha-maker':'animal-captcha-maker.php').'" method="POST">
	...
	<h2>Crop image</h2>		
	<form action="'.($in_wp?''.basename(dirname(__FILE__)).'/wp-admin/plugins.php?page=animal-captcha-maker':'animal-captcha-maker.php').'" method="POST" style="display:inline;">
	...
	</form> <button onclick="window.location.href=\''.($in_wp?'plugins.php?page=animal-captcha-maker&':'animal-captcha-maker.php?').'a=borrar&imagen='.$final_name_root.'\';" style="color:red;display:inline;">Delete</button><br />
	...
	';
			...
			case 'process_img':
				...
				$txt_accion = '<em>Image created: '.$src.'</em> <button onclick="window.location.href=\''.($in_wp?'plugins.php?page=animal-captcha-maker&':'animal-captcha-maker.php?').'a=borrar&imagen='.$src.'\';" style="color:red;display:inline;">Undo</button>';
				break;
	...
	<button onclick="window.location.href=\''.($in_wp?'plugins.php?page=animal-captcha-maker&':'animal-captcha-maker.php?').'a=add_image\';" style="font-weight:bold;">Create new</button> 
	(<a href="'.($in_wp?''. basename(dirname(__FILE__)).'/wp-content/plugins/animal-captcha/source/example.php':'example.php').'"><b>Try test</b></a>) 
	(<a href="http://gonzo.teoriza.com/animal-captcha">Send opinion or problem</a>)
	</span>
	<table border="0" cellpadding="0" cellspacing="1">';
	foreach ($animals_array AS $animal => $none1) {
		echo '<form accion="'.($in_wp?''. basename(dirname(__FILE__)).'/wp-admin/plugins.php?page=animal-captcha-maker':'animal-captcha-maker.php').'" method="POST"><input type="hidden" name="a" value="mod_name" /><input type="hidden" name="name" value="'.$animal.'" /><tr><td align="right">';
		$num_animals = 0;
	...
	<td valign="top" style="color:#AAA;"><input type="submit" value="Edit" /></form> <button onclick="window.location.href=\''.($in_wp?'plugins.php?page=animal-captcha-maker&':'animal-captcha-maker.php?').'a=add_image&animal='.$animal.'\';" style="font-weight:bold;">Add</button><br />
